...
export const NotesEditor = memo(
  function NotesEditor({ onTimeClick, onNewMeeting, isRecording, onToggleRecording }: Props) {
    const {
      title,
      setTitle,
      notes,
    ...
    const { saveToNotion, isSavingToNotion, isNotionConnected } = useNotionIntegration();
    ...
    useEffect(() => {
      function handleClickOutside(event: MouseEvent) {
        ...
        if (
          editingTime &&
          editTimeRef.current &&
          !editTimeRef.current.contains(event.target as Node)
        ) {
    ...
    const handleGenerateNotes = async () => {
      ...
      try {
        ...
        const updatedNotes = [...notes, summaryNote];
        await setNotes(updatedNotes);

        // Push to Notion if connected
        if (isNotionConnected) {
          try {
            const formattedNotes = updatedNotes
              .map((note) => {
                const time = note.timestamp.toLocaleTimeString();
                return note.isAiGenerated ? note.text : `[${time}] ${note.text}`;
              })
              .join("\n\n");
            ...
            const result = await saveToNotion(
              ...
              formattedNotes,
              data.notionPageId // Pass existing page ID if it exists
            );

            // Store the page ID for future updates
            if (result.pageId && !data.notionPageId) {
              await updateStore({
                ...data,
            ...
            toast({
              title: "Notes updated",
              description: "Meeting notes and summary saved to Notion",
            });
          } catch (error) {
        ...
        toast({
          ...
          description: "AI summary has been added to your notes",
        });
      } catch (error) {
    ...
    const handleImprove = async (note: Note) => {
      console.log("improving note with context:", {
        ...
        relevant_segments: segments.filter(
          (s) => new Date(s.timestamp).getTime() <= note.timestamp.getTime()
        ),
      });

      const lastSegmentTime =
        segments.length > 0 ? new Date(segments[segments.length - 1].timestamp).getTime() : 0;
      ...
      const context = `[${relevantSegment.timestamp}] [${relevantSegment.speaker ?? "unknown"}]: ${
        relevantSegment.transcription
      }`;
      ...
      <div className="h-full flex flex-col bg-card relative">
        ...
        `}
          ...
          <div className="p-1 rounded-md bg-gray-100/80 backdrop-blur hover:bg-gray-200/80 transition-colors">
            <ChevronDown
              className={`h-5 w-5 text-gray-400 transition-transform duration-200 hover:text-gray-600 
            ${showNav ? "rotate-180" : ""}`}
            />
          </div>
        </div>
              ...
              >
                {sortedNotes.map((note) => (
                  ...
                  >
                    ...
                    {editingId === note.id ? (
                      <div ref={editRef} className="mt-1">
                        <input
                          type="text"
