...
import { ArrowDown, Layout, LayoutList, Loader2 } from "lucide-react";
  ...
  return (
    <>
      {diffs.map((diff, i) => {
        ...
        if (diff.removed) {
          return (
            <span key={i} className="bg-red-100 line-through">
              {diff.value}
            </span>
          );
        }
...
export function TranscriptionView({ isLoading, settings }: TranscriptionViewProps) {
  ...
  const { saveToNotion, isSavingToNotion, notionError, isNotionConnected } = useNotionIntegration();
  ...
  const handleGenerateNote = async (chunk: TranscriptionChunk) => {
    try {
      ...
      setNotes([
        ...notes,
        { id: crypto.randomUUID(), text: note, timestamp: new Date(chunk.timestamp) },
      ]);
    } catch (error) {
  ...
  const handleClose = async () => {
    try {
      if (!isNotionConnected) {
        console.warn("Notion is not connected");
        return;
      }
      ...
      await saveToNotion(`Meeting Notes - ${new Date().toLocaleDateString()}`, notesContent);

      // Then proceed with existing close logic
      // ... existing close logic ...
    } catch (error) {
      console.error("Failed to save to Notion:", error);
      // Error will be shown via StatusAlerts component
    }
  };
    ...
    <>
          ...
          {data?.chunks && data.chunks.length > 0 && (
            <div className="space-y-2 relative p-4">
              ...
              {mergedChunks.map((chunk) => (
                <div key={chunk.id} className="text-sm mb-2 group relative">
                  {viewMode === "overlay" ? (
                    <>
                      <ChunkOverlay
                        ...
                        displaySpeaker={
                          chunk.speaker ? getDisplaySpeaker(chunk.speaker) : "speaker_0"
                        }
                        onSpeakerClick={() => {
                          if (chunk.speaker) {
                      ...
                      <div className="relative">
                        <div
                          className={cn(
                            ...
                            improvingChunks[chunk.id] &&
                              "animate-shimmer bg-gradient-to-r from-transparent via-gray-100/50 to-transparent bg-[length:200%_100%]",
                    ...
                    <div className="flex gap-1">
                      ...
                      <div className="outline-none rounded flex-1">{chunk.text}</div>
                    </div>
                  ) : (
                    <div className="flex gap-1">
      ...
      <Dialog open={mergeModalOpen} onOpenChange={setMergeModalOpen}>
        <DialogContent className="sm:max-w-md max-h-[70vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>
              Rename or merge {formatSpeaker(getDisplaySpeaker(selectedSpeaker!))}
            </DialogTitle>
          </DialogHeader>
