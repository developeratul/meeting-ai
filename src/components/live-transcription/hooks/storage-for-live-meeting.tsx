...
import { MeetingSegment, Note, TranscriptionChunk } from "../../meeting-history/types";
import { MeetingAnalysis } from "./ai-create-all-notes";
import { createHandleNewChunk, ImprovedChunk } from "./handle-new-chunk";
  ...
  const value = useMemo(
    () => ({
      ...
      segments: (data?.chunks || []).map((chunk) => ({
        ...
        speaker:
          data?.speakerMappings[chunk.speaker || "speaker_0"] || chunk.speaker || "speaker_0",
      })),
      setSegments: async () => {},
